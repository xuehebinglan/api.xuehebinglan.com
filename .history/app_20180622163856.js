...
app.get('/drinkwater/*', function (req, res) {
  ...
  if (req.url.indexOf('/getdata') > -1) {
    ...
    routerFunctions.drinkwater.initUserData(req.query.username, req.query.date, req.query.cup_capacity, req.query.operation_type).then((data) => {
      res.send(data)
    })
  } else if (req.url.indexOf('/setdata') > -1) {
    console.log('/setdata')
    routerFunctions.drinkwater.setUserData(req.query.username, req.query.date, req.query.cup_capacity, req.query.operation_type).then((data) => {
      res.send(data)
    })
