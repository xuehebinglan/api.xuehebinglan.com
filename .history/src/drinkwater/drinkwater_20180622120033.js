  ...
  setDrinkCupCapCapacity(options) {
    tool.getJSON('drinkwater').then((data) => {
      ...
      if (options.operationType === 'sub') {
        allDrinkData[username][date].total_cup_number -= 1
        allDrinkData[username][date].cup_capacity = allDrinkData[username][date].record_drink.pop()
        allDrinkData[username][date].total_drink_water -= allDrinkData[username][date].cup_capacity
      }
